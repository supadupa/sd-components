...
/*
// semi custom jQuery wrapper for the spin.js library above.
$('#el').spin({lines:4});
$('#el').spin(false);
*/
$.fn.spin = function(opts) {
  var default_opts = { lines: 12, length: 3, width: 4, radius: 6, speed: 1, trail: 60, shadow: false, color: '#000' };
  if (Spinner) {
    return this.each(function() {
      ...
      if (opts !== false) {
        data.spinner = new Spinner($.extend(default_opts, opts)).spin(this);
      }
    });
  } else {
